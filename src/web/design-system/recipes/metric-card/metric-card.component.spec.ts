...
class MetricCardTestHostComponent {
  readonly value = signal('123,456,789,012,345 units completed');
  readonly trend = signal<MetricTrend>({ label: '12% higher than previous period', direction: 'up' });
...
describe('MetricCardComponent', () => {
  ...
  it('renders caller-formatted values and trend meaning as text', () => {
    ...
    expect(card.querySelector('lsd-icon svg')?.getAttribute('aria-hidden')).toBe('true');
  });
