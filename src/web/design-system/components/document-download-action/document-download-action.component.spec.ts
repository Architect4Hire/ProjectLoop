...
class Host { state: DocumentDownloadState = 'ready'; intent: DocumentDownloadIntent | null = null; }
...
describe('DocumentDownloadActionComponent', () => {
  ...
  it('announces preparing, downloading, failed, and unavailable states', () => {
    ...
    for (const [state, text, role] of expected) {
      host.state = state; fixture.detectChanges();
  ...
  it('renders native progress semantics while downloading', () => {
    host.state = 'downloading'; fixture.detectChanges(); const progress = fixture.debugElement.query(By.css('progress')).nativeElement as HTMLProgressElement;
  ...
  it('emits retry intent only from failed state', () => {
    host.state = 'failed'; fixture.detectChanges(); fixture.debugElement.query(By.css('button')).nativeElement.click();
