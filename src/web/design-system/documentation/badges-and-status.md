...
The document-status token maps the complete presentation vocabulary `draft | published | superseded | archived | unavailable` to existing Badge variants. It defines no lifecycle transitions or domain behavior.
...
const presentation = documentStatusPresentation('published', labels);
```
